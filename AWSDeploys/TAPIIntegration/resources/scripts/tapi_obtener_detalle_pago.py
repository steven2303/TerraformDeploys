...
def lambda_handler(event, context):
    ...
    try:
        # Obtiene detalles de la compañía
        company_details = tapi_instance.get_companie_detail(company_code=company_code)
        if 'error' in company_details:
            message = company_details['error']['message']
            data = ''
            log_message = message
            codigo_salida = 500
        else:
            # Realiza el pago con los parámetros proporcionados
            company_code = company_details['recharge']['companyCode']
            product_id = company_details['recharge']['products'][-1]['productId']
            identifier_name = company_details['recharge']['identifierName']
