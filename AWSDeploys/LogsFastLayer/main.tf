module "glue_module" {
  ...
  glue_audit_log_script_location_s3 = "s3://${var.s3_resources_bucket_name}/scripts/AuditLogDataETL.py"
  ...
  upload_scripts_arn = module.s3_module.upload_scripts_arn
  audit_log_input_full_path = "s3://${var.audit_log_landing_bucket_name}/${var.full_audit_log_key_path}"
  audit_log_output_path = "s3://${module.s3_module.s3_audit_logs_processed_bucket_name}/${var.glue_audit_log_crawler_paths[0]}"
  audit_log_error_path = "s3://${module.s3_module.s3_audit_logs_processed_bucket_name}/${var.glue_s3_audit_log_error_location}/historico"
  providers = {
    aws = aws.virginia
  }
}
